...
namespace Lets.Be
{
    static partial class Rational
    {
        ...
        const double DBL_EPSILON = 1E-16;
        const double DBL_MIN = double.Epsilon;
        const double DBL_MAX = double.MaxValue;

        const double TWO_PI                        = 6.283185307179586476925286766559005768394338798750;
        const double SQRT_PI_OVER_TWO              = 1.253314137315500251207882642405522626503493370305;  // sqrt(pi/2) to avoid misinterpretation.
        const double SQRT_THREE                    = 1.732050807568877293527446341505872366942805253810;
        const double SQRT_ONE_OVER_THREE           = 0.577350269189625764509148780501957455647601751270;
        const double TWO_PI_OVER_SQRT_TWENTY_SEVEN = 1.209199576156145233729385505094770488189377498728; // 2*pi/sqrt(27)
        const double PI_OVER_SIX                   = 0.523598775598298873077107230546583814032861566563;

        static readonly double SQRT_DBL_EPSILON = Math.Sqrt(DBL_EPSILON);
        static readonly double FOURTH_ROOT_DBL_EPSILON = Math.Sqrt(SQRT_DBL_EPSILON);
        static readonly double EIGHTH_ROOT_DBL_EPSILON = Math.Sqrt(FOURTH_ROOT_DBL_EPSILON);
        static readonly double SIXTEENTH_ROOT_DBL_EPSILON = Math.Sqrt(EIGHTH_ROOT_DBL_EPSILON);
        static readonly double SQRT_DBL_MIN = Math.Sqrt(DBL_MIN);
        static readonly double SQRT_DBL_MAX = Math.Sqrt(DBL_MAX);
        ...
        const double DENORMALIZATION_CUTOFF = 0;

        const double VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_BELOW_INTRINSIC = -DBL_MAX;
        const double VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_ABOVE_MAXIMUM = DBL_MAX;

        static bool is_below_horizon(double x) { return Math.Abs(x) < DENORMALIZATION_CUTOFF; } // This weeds out denormalized (a.k.a. 'subnormal') numbers.
...
//#endif
        ...
        static atomic_t implied_volatility_maximum_iterations = new atomic_t { data = 2 }; // (DBL_DIG*20)/3 ≈ 100 . Only needed when the iteration effectively alternates Householder/Halley/Newton steps and binary nesting due to roundoff truncation.

        #if ENABLE_SWITCHING_THE_OUTPUT_TO_ITERATION_COUNT
        static atomic_t implied_volatility_output_type = new atomic_t { data = 0 };
        static double implied_volatility_output(int count, double volatility) { return implied_volatility_output_type.data > 0 ? count : volatility; }
        #else
        static double implied_volatility_output(int count, double volatility) { return volatility; }
        #endif

        #if ENABLE_CHANGING_THE_HOUSEHOLDER_METHOD_ORDER
        static atomic_t implied_volatility_householder_method_order = new atomic_t { data = 4 };
        static double householder_factor(double newton, double halley, double hh3)
        {
        ...
        #else
        static double householder_factor(double newton, double halley, double hh3) { return (1 + 0.5 * halley * newton) / (1 + newton * (halley + hh3 * newton / 6)); }
        #endif

        public static double set_implied_volatility_maximum_iterations(double t)
        {
            int i = (int)t;
            if (i >= 0)
            {
                Interlocked.Exchange(ref implied_volatility_maximum_iterations.data, i);
            }
            return implied_volatility_maximum_iterations.data;
        }

        #if ENABLE_SWITCHING_THE_OUTPUT_TO_ITERATION_COUNT
        public static double set_implied_volatility_output_type(double t)
        {
            int i = (int)t;
            Interlocked.Exchange(ref implied_volatility_output_type.data, i);
            return implied_volatility_output_type.data;
        }
        #endif  

        #if ENABLE_CHANGING_THE_HOUSEHOLDER_METHOD_ORDER
        public static double set_implied_volatility_householder_method_order(double t)
        {
            int i = (int)t;
            if (i >= 0)
            {
                Interlocked.Exchange(ref implied_volatility_householder_method_order.data, i);
            }
            return implied_volatility_householder_method_order.data;
        }
        #endif  

        static double normalised_intrinsic(double x, double q /* q=±1 */)
        {
            if (q * x <= 0)
                return 0;
            double x2 = x * x;
        ...
        static double normalised_intrinsic_call(double x) { return normalised_intrinsic(x, 1); }
        ...
        static double asymptotic_expansion_of_normalized_black_call(double h, double t)
        {
        ...
        static double small_t_expansion_of_normalized_black_call(double h, double t)
        {
        ...
        /* τ */ static readonly double small_t_expansion_of_normalized_black_threshold = 2 * SIXTEENTH_ROOT_DBL_EPSILON;

        //     b(x,s)  =  Φ(x/s+s/2)·exp(x/2)  -   Φ(x/s-s/2)·exp(-x/2)
        //             =  Φ(h+t)·exp(x/2)      -   Φ(h-t)·exp(-x/2)
        // with
        //              h  =  x/s   and   t  =  s/2
        static double normalized_black_call_using_norm_cdf(double x, double s)
        {
            double h = x / s, t = 0.5 * s, b_max = Math.Exp(0.5 * x), b = norm_cdf(h + t) * b_max - norm_cdf(h - t) / b_max;
            return Math.Abs(Math.Max(b, 0.0));
        }
        ...
        public static double normalised_black(double x, double s, double q /* q=±1 */) { return normalised_black_call(q < 0 ? -x : x, s); /* Reciprocal-strike call-put equivalence */ }
        ...
        #if COMPUTE_LOWER_MAP_DERIVATIVES_INDIVIDUALLY
        static double f_lower_map(const double x,const double s)
        {
        ...
        static double d_f_lower_map_d_beta(const double x,const double s)
        {
            if (is_below_horizon(s))
                return 1;
        ...
        static double d2_f_lower_map_d_beta2(const double x,const double s)
        {
        ...
        static void compute_f_lower_map_and_first_two_derivatives(const double x,const double s, double &f, double &fp, double &fpp)
        {
            f = f_lower_map(x, s);
        ...
        #else
        static void compute_f_lower_map_and_first_two_derivatives(double x, double s, out double f, out double fp, out double fpp)
        {
            double ax = Math.Abs(x), z = SQRT_ONE_OVER_THREE * ax / s, y = z * z, s2 = s * s, Phi = norm_cdf(-z), phi = norm_pdf(z);
        ...
        static double inverse_f_lower_map(double x, double f)
        {
            return is_below_horizon(f) ? 0 : Math.Abs(x / (SQRT_THREE * inverse_norm_cdf(Math.Pow(f / (TWO_PI_OVER_SQRT_TWENTY_SEVEN * Math.Abs(x)), 1.0/ 3.0))));
        }

        #if COMPUTE_UPPER_MAP_DERIVATIVES_INDIVIDUALLY
        static double f_upper_map(const double s)
        {
            return norm_cdf(-0.5 * s);
        }
        static double d_f_upper_map_d_beta(const double x,const double s)
        {
            return is_below_horizon(x) ? -0.5 : -0.5 * exp(0.5 * square(x / s));
        }
        static double d2_f_upper_map_d_beta2(const double x,const double s)
        {
            if (is_below_horizon(x))
                return 0;
        ...
        static void compute_f_upper_map_and_first_two_derivatives(const double x,const double s, double &f, double &fp, double &fpp)
        {
            f = f_upper_map(s);
        ...
        #else
        static void compute_f_upper_map_and_first_two_derivatives(double x, double s, out double f, out double fp, out double fpp)
        {
            f = norm_cdf(-0.5 * s);
            if (is_below_horizon(x))
            {
        ...
        static double inverse_f_upper_map(double f)
        {
        ...
        //
        static double unchecked_normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(double beta, double x, double q /* q=±1 */, int N)
        {
            // Subtract intrinsic.
            if (q * x > 0)
            {
            ...
            if (beta < b_c)
            {
                ...
                if (beta < b_l)
                {
                    ...
                    double r_ll = convex_rational_cubic_control_parameter_to_fit_second_derivative_at_right_side(0.0, b_l, 0.0, f_lower_map_l, 1.0, d_f_lower_map_l_d_beta, d2_f_lower_map_l_d_beta2, true);
                ...
                {
                    double v_l = normalised_vega(x, s_l), r_lm = convex_rational_cubic_control_parameter_to_fit_second_derivative_at_right_side(b_l, b_c, s_l, s_c, 1 / v_l, 1 / v_c, 0.0, false);
            ...
            else
            {
                ...
                if (beta <= b_h)
                {
                    double v_h = normalised_vega(x, s_h), r_hm = convex_rational_cubic_control_parameter_to_fit_second_derivative_at_left_side(b_c, b_h, s_c, s_h, 1 / v_c, 1 / v_h, 0.0, false);
                ...
                else
                {
                    ...
                    {
                        double r_hh = convex_rational_cubic_control_parameter_to_fit_second_derivative_at_left_side(b_h, b_max, f_upper_map_h, 0.0, d_f_upper_map_h_d_beta, -0.5, d2_f_upper_map_h_d_beta2, true);
        ...
        public static double implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(double price, double F, double K, double T, double q /* q=±1 */, int N)
        {
            double intrinsic = Math.Abs(Math.Max((q < 0 ? K - F : F - K), 0.0));
            if (price < intrinsic)
        ...
        public static double implied_volatility_from_a_transformed_rational_guess(double price, double F, double K, double T, double q /* q=±1 */)
        {
        ...
        public static double normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(double beta, double x, double q /* q=±1 */, int N)
        {
            // Map in-the-money to out-of-the-money
            if (q * x > 0)
            {
        ...
        public static double normalised_implied_volatility_from_a_transformed_rational_guess(double beta, double x, double q /* q=±1 */)
        {
